...
public class Student extends Users {
    ...
    public boolean registerToCourse(Course course) throws IOException {
        ...
        if (this.getTranscript().getSemesters().isEmpty()) {
            this.getTranscript().getSemesters().add(new Semester(1));
        }

        List<Semester> semesters = this.getTranscript().getSemesters();
        Semester lastSemester = semesters.get(semesters.size() - 1);

        if (lastSemester.getCredits() + course.getCredits() > 21) return false;

        this.getCourses().add(course);
        lastSemester.addCredits(course.getCredits());
